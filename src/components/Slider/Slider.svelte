<script>
	...
	export let styleType = undefined;
	export let idInput = undefined;
	export let nameInput = undefined;
...
<div class="container">
	<div class="container-input range-wrap">
		<div
			class="slider-value range-value {styleType == 'primary'
				? 'primary'
				: 'secondary'}"
		>
			{#if valueInput && valueInput !== undefined}
				<output
					for={nameInput}
					class="value-input {visibility ? 'visible' : 'noShow'} {styleType ==
					'primary'
						? 'primary'
						: 'secondary'}"
					style="left:{valueLeftOnChanges}"
					>{valueInput}
				</output>
			{/if}
		</div>
