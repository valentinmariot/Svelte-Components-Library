<script>
	...
	function password_checker() {
		if (
			points_assignment(password_write) >= 5 &&
			points_assignment(password_write) <= 9
		) {
			face_humor = 2;
			change_face();
...
<div id="input_face">
	<div class="input">
		<input
			...
			id="input_password"
			bind:value={password_write}
			placeholder="@Passw0rd"
			on:input={password_checker}
		/>
	...
	<div
		id="face-icon"
		class="face"
		style="display: {face && password_write ? 'block' : 'none'};
	...
	>
		...
		<div class="mouth {class_mouth_humor}" id="mouth" />
	</div>
	<span
		id="text_message_humor"
		style="color: {color_text_message_humor};
        display: {face && password_write ? 'block' : 'none'}"
		>{text_message_humor}</span
	>
</div>
{#if password_write}
	<div id="checked_boxes">
		<div
			class="checkbox"
			id="uppercase"
			style="display: {uppercase ? 'block' : 'none'}"
		>
			<input
				type="checkbox"
				disabled="disabled"
				name="uppercase"
				checked={check_uppercase(password_write)}
			/>
		...
		<div
			class="checkbox"
			id="special_charact"
			style="display: {special_charact ? 'block' : 'none'}"
		>
			<input
				type="checkbox"
				disabled="disabled"
				name="special_charact"
				checked={check_special(password_write)}
			/>
		...
		<div class="checkbox" style="display: {number ? 'block' : 'none'}">
			<input
				...
				name="number"
				id="number"
				checked={check_number(password_write)}
			/>
